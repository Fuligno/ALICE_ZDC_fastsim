...
# 4) Impact point (2x2): 2D hist pixel + overlay 1D (x,y)
# 5) Occupancy (centered) — real vs gen + profili X/Y
# 6) Sum (centered) — real vs gen + profili X/Y
# ============================================================
...
def imshow_with_individual_colorbar(ax, img2d, title: str, xlabel: str, ylabel: str,
    ...
    if mid_lines is not None:
        ...
        ax.axhline(my, color="red", linestyle="--", linewidth=1.0)
    divider = make_axes_locatable(ax); cax = divider.append_axes("right", size="3.5%", pad=0.06)
    cb = plt.colorbar(im, cax=cax); cb.set_label("counts", fontsize=9); cb.ax.tick_params(labelsize=8)
...
def center_images_fft(imgs: np.ndarray, bx01: np.ndarray, by01: np.ndarray, device: torch.device | None = None) -> np.ndarray:
    ...
    # FIX: uso to(device=..., dtype=...) per evitare l'errore TypeError
...
def main():
    ...
    ap.add_argument("--in", dest="in_path", default="/data/dataalice/dfuligno/ZDC_fastsim//build_data/CFM_vanilla.parquet",
                    help="Parquet di input (es. CFM_vanilla.parquet)")
    ap.add_argument("--out-dir", default="CFM_vanilla",
    ...
    # FIX: aggiungo qui (prima del parse) i due argomenti
    ...
    imshow_with_individual_colorbar(axs[0,0], occ_real, "Real — occupancy (centered)",
                                    "x (pixel)", "y (pixel)", pixel_axes=True,
                                    mid_lines=(MID_X, MID_Y))
    imshow_with_individual_colorbar(axs[0,1], occ_gen,  "Generated — occupancy (centered)",
                                    "x (pixel)", "y (pixel)", pixel_axes=True,
                                    mid_lines=(MID_X, MID_Y))
    # profili
    ...
    save_figure(fig, out_dir / "centered_occupancy_2x2.png",
                "centered occupancy maps & projections", args.dpi)

    # ==================== Canvas 6: SUM (centered) ====================
    ...
    fig, axs = plt.subplots(2, 2, figsize=(12.2, 8.8), constrained_layout=False)
    imshow_with_individual_colorbar(axs[0,0], sum_real, "Real — sum (centered)",
                                    "x (pixel)", "y (pixel)", pixel_axes=True,
                                    mid_lines=(MID_X, MID_Y))
    imshow_with_individual_colorbar(axs[0,1], sum_gen,  "Generated — sum (centered)",
                                    "x (pixel)", "y (pixel)", pixel_axes=True,
                                    mid_lines=(MID_X, MID_Y))
    # profili
    ...
    save_figure(fig, out_dir / "centered_sum_2x2.png",
                 args.dpi)
        # ==================== Canvas 7: Random samples (real vs generated) ====================
    ...
    for k, idx in enumerate(sel):
        ...
        ax_r.set_title(f"Real #{k+1}", fontsize=10, pad=4)
        _fmt_pixel_axis(ax_r, "both")
        ax_r.set_xlabel("x [Pixel]")
        ax_r.set_ylabel("y [Pixel]")
        div_r = make_axes_locatable(ax_r)
        cax_r = div_r.append_axes("right", size="3%", pad=0.05)
        cb_r = plt.colorbar(im_r, cax=cax_r)
        cb_r.ax.tick_params(labelsize=8)

        im_g = ax_g.imshow(g, origin="lower", interpolation="nearest", cmap="viridis")
        ax_g.set_title(f"Gen #{k+1}", fontsize=10, pad=4)
        _fmt_pixel_axis(ax_g, "both")
        ax_g.set_xlabel("x [Pixel]")
        ax_g.set_ylabel("y [Pixel]")
        div_g = make_axes_locatable(ax_g)
        cax_g = div_g.append_axes("right", size="3%", pad=0.05)
        cb_g = plt.colorbar(im_g, cax=cax_g)
        cb_g.ax.tick_params(labelsize=8)

    save_figure(fig, out_dir / "samples_real_vs_gen.png", args.dpi)

    print(f"[OK] Salvate 7 figure in: {out_dir.resolve()}")
