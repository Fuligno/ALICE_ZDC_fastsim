#!/usr/bin/env python3
# ============================================================
# single_model_plots.py — modelli "singoli" (CFM vanilla & co.)
# ============================================================
...
def _root_like_colormap() -> LinearSegmentedColormap:
    # Blu -> ciano -> verde -> giallo (tipo "kBird-like")
    colors = [
    ...
    # zero trasparente gestito via masked array + set_bad(alpha=0)
    cmap.set_bad(alpha=0.0)
    return cmap

# ---------- Colorbar a destra di un asse (solo per il pannello sinistro) ----------
...
def _hist2d_mappable(ax, x, y, bins=(80, 80), xrange=None, yrange=None,
    ...
    # Calcolo manuale per poter mascherare gli zeri (trasparenti)
    Hh, xe, ye = np.histogram2d(x, y, bins=bins, range=[xrange, yrange])
    data = np.ma.masked_less_equal(Hh.T, 0.0)  # zeri mascherati => trasparenti
    ...
    return im, Hh  # ritorno anche i conteggi crudi se servono
...
def hist1d_overlay(ax, real, gen, bins_edges, xlabel: str, title: str,
    ...
    # Niente label x (verrà nel ratio)
    ax.set_xlabel("")
    ...
    ax.tick_params(axis="both", labelsize=9)
    ax.tick_params(axis="x", labelbottom=False)  # niente labels x in alto

def ratio_panel(ax_ratio, bin_centers, real_counts, gen_counts,
                ylabel="Gen / Real", xlabel="", ylim=(0.0, 2.0)):
    denom = np.clip(real_counts, EPS, None)
    ratio = gen_counts / denom
    ax_ratio.plot(bin_centers, ratio, linewidth=1.6)
    ax_ratio.axhline(1.0, linestyle="--", linewidth=1.0, color="black")
    ax_ratio.set_ylim(*ylim)
    ax_ratio.set_ylabel(ylabel)
    ax_ratio.set_xlabel(xlabel)
    ax_ratio.grid(alpha=0.2, linestyle=":")
    _fmt_sci(ax_ratio, "x")
...
def overlay_plus_ratio(col_ax_top: plt.Axes, col_ax_bottom: plt.Axes,
    ...
    if pixel_axis: _fmt_pixel_axis(col_ax_top, "x")
    else: _fmt_sci(col_ax_top, "x")
    if mid_line is not None:
    ...
    # bottom: ratio (0..2) e pannelli attaccati
    ...
    if pixel_axis: _fmt_pixel_axis(col_ax_bottom, "x")
    else: _fmt_sci(col_ax_bottom, "x")
...
# ---------- centratura ----------
def center_images_fft(imgs: np.ndarray, bx01: np.ndarray, by01: np.ndarray, device: torch.device | None = None) -> np.ndarray:
    """
...
def _wd_1d(a: np.ndarray, b: np.ndarray) -> float:
    ...
    # Prova scipy, altrimenti fallback quantile-approx
    try:
...
def _sliced_wd_2d(P: np.ndarray, Q: np.ndarray, n_proj: int = 128, seed: int = 42) -> float:
    """
    P,Q: (N,2) punti (non pesati). Ritorna media delle WD(1D) su proiezioni random.
    """
    rng = np.random.default_rng(seed)
    if P.size == 0 and Q.size == 0: return 0.0
    if P.ndim != 2 or Q.ndim != 2 or P.shape[1] != 2 or Q.shape[1] != 2:
        raise ValueError("P e Q devono essere (N,2)")
    ...
    for u in thetas:
        p1 = P @ u
        q1 = Q @ u
        vals.append(_wd_1d(p1, q1))
    return float(np.mean(vals))

def _sample_points_from_weighted_grid(grid: np.ndarray, max_samples: int = 30000, seed: int = 42) -> np.ndarray:
    """
    grid: [H,W] non-negativa. Campiona punti (x,y) con probabilità proporzionale a grid[y,x].
    """
    ...
    # coordinate in "pixel" (0..W-1, 0..H-1)
    pts = np.stack([xs.astype(np.float64), ys.astype(np.float64)], axis=1)
    return pts

# ---------- Entropia per matrice ----------
def _entropy_per_image(imgs: np.ndarray) -> Tuple[float, float]:
    """
    imgs: [N,H,W], non-negativi.
    Ritorna (mean, var) delle entropie -sum p log p (base e).
    """
    ent = []
    for m in imgs:
        s = float(m.sum())
...
# ---------- NUOVA metrica: Sliced WD in alta dimensione su immagini intere ----------
def _sliced_wd_images(reals: np.ndarray, gens: np.ndarray, n_proj: int = 256, max_n: int = 4000, seed: int = 0) -> float:
    """
    Sliced Wasserstein tra DISTRIBUZIONI di immagini (ogni immagine è un punto in R^(H*W)).
    reals/gens: [N,H,W]
    n_proj: numero di direzioni casuali in alta dimensione.
    max_n: sottocampiona fino a max_n immagini per lato (per efficienza).
    """
    rng = np.random.default_rng(seed)
    A = reals.reshape(reals.shape[0], -1).astype(np.float32, copy=False)  # [N,D]
    B = gens.reshape(gens.shape[0], -1).astype(np.float32, copy=False)    # [M,D]
    ...
    # WD 1D per ciascuna proiezione (quantile-matching tramite ordinamento)
    vals = []
    for j in range(PA.shape[1]):
        a = np.sort(PA[:, j])
        b = np.sort(PB[:, j])
        # EMD 1D = media delle distanze assolute tra i quantili ordinati
        vals.append(float(np.mean(np.abs(a - b))))
    return float(np.mean(vals) if vals else 0.0)

# ---------- main ----------
def main():
    ap = argparse.ArgumentParser(description="Plot macro per modelli singoli (CFM vanilla & co.)")
    ap.add_argument("--in", dest="in_path", default="/data/dataalice/dfuligno/ZDC_fastsim/build_data/DDPM_guided.parquet",
                    help="Parquet di input (es. CFM_vanilla.parquet)")
    ap.add_argument("--out-dir", default="DDPM_guided",
    ...
    # ---- nuovi flag per SWD immagini intere ----
    ...
    needed = ["psum_true","psum_gen","pmax_true","pmax_gen",
              "bary_x_true","bary_y_true","bary_x_gen","bary_y_gen",
              "real","gen"]
    for c in needed:
    ...
    # -------- Canvas 3A: 2D hist (Real vs Gen) con scala comune --------
    ...
    # costruiamo le 2 mappe + norm comune su massimo globale
    H1, xe1, ye1 = np.histogram2d(psum_true, pmax_true, bins=(args.bins2d, args.bins2d), range=[xrange, yrange])
    H2, xe2, ye2 = np.histogram2d(psum_gen,  pmax_gen,  bins=(args.bins2d, args.bins2d), range=[xrange, yrange])
    ...
    bx_t_px = (df["bary_x_true"].to_numpy(np.float64, copy=False)) * (W - 1.0)
    by_t_px = (df["bary_y_true"].to_numpy(np.float64, copy=False)) * (H - 1.0)
    bx_g_px = (df["bary_x_gen"].to_numpy(np.float64,  copy=False)) * (W - 1.0)
    by_g_px = (df["bary_y_gen"].to_numpy(np.float64,  copy=False)) * (H - 1.0)
    ...
    # precompute vmax per norm
    Hb1, xeb1, yeb1 = np.histogram2d(bx_t_px, by_t_px, bins=bins2d_bary, range=[xy_range[0], xy_range[1]])
    Hb2, xeb2, yeb2 = np.histogram2d(bx_g_px, by_g_px, bins=bins2d_bary, range=[xy_range[0], xy_range[1]])
    ...
    # layout 2 colonne, ciascuna con top+bottom (ratio) — hspace=0 per pannelli attaccati
    fig = plt.figure(figsize=(12.2, 7.6))
    gs = fig.add_gridspec(ncols=2, nrows=2, height_ratios=[3,1], wspace=0.28, hspace=0.0)
    ax_x_top   = fig.add_subplot(gs[0,0])
    ax_x_ratio = fig.add_subplot(gs[1,0], sharex=ax_x_top)
    ax_y_top   = fig.add_subplot(gs[0,1])
    ax_y_ratio = fig.add_subplot(gs[1,1], sharex=ax_y_top)
    ...
    # ==================== Canvas 5A: OCCUPANCY (centered) — solo 2D ====================
    reals = _stack_images(df["real"])
    gens  = _stack_images(df["gen"])

    bx_t01 = df["bary_x_true"].to_numpy(np.float32, copy=False)
    by_t01 = df["bary_y_true"].to_numpy(np.float32, copy=False)
    bx_g01 = df["bary_x_gen"].to_numpy(np.float32,  copy=False)
    by_g01 = df["bary_y_gen"].to_numpy(np.float32,  copy=False)

    reals_c = center_images_fft(reals, bx_t01, by_t01)
    gens_c  = center_images_fft(gens,  bx_g01, by_g01)

    # occupancy: +1 se pixel >= 1, altrimenti 0
    occ_real = (reals_c >= 1.0).sum(axis=0).astype(np.float64)
    occ_gen  = (gens_c  >= 1.0).sum(axis=0).astype(np.float64)
    ...
    imL = _imshow_mappable(axs[0], occ_real, "Real — occupancy (centered)",
                           "x [Pixel]", "y [Pixel]", pixel_axes=True,
                           mid_lines=(MID_X, MID_Y), cmap=cmap, norm=norm_occ)
    imR = _imshow_mappable(axs[1], occ_gen,  "Generated — occupancy (centered)",
                           "x [Pixel]", "y [Pixel]", pixel_axes=True,
                           mid_lines=(MID_X, MID_Y), cmap=cmap, norm=norm_occ)
    _add_cbar_right_of(axs[0], imL, label="Counts")
    save_figure(fig, out_dir / "centered_occupancy_2D_only.png", args.dpi)
    ...
    overlay_plus_ratio(ax_ytop, ax_yratio, ys, projy_real, projy_gen,
                       ...
                       pixel_axis=True, mid_line=MID_Y)
    save_figure(fig, out_dir / "centered_occupancy_profiles_ratio.png", args.dpi, tight=False)

    # ==================== Canvas 6A: SUM (centered) — solo 2D ====================
    sum_real = reals_c.sum(axis=0).astype(np.float64)
    sum_gen  = gens_c.sum(axis=0).astype(np.float64)

    fig, axs = plt.subplots(1, 2, figsize=(12.2, 4.8), constrained_layout=False)
    vmax_sum = float(max(sum_real.max(initial=0.0), sum_gen.max(initial=0.0)))
    norm_sum = Normalize(vmin=0.0, vmax=max(vmax_sum, 1.0))
    imL = _imshow_mappable(axs[0], sum_real, "Real — sum (centered)",
                           "x [Pixel]", "y [Pixel]", pixel_axes=True,
                           mid_lines=(MID_X, MID_Y), cmap=cmap, norm=norm_sum)
    imR = _imshow_mappable(axs[1], sum_gen,  "Generated — sum (centered)",
                           "x [Pixel]", "y [Pixel]", pixel_axes=True,
                           mid_lines=(MID_X, MID_Y), cmap=cmap, norm=norm_sum)
    _add_cbar_right_of(axs[0], imL, label="sum")
    save_figure(fig, out_dir / "centered_sum_2D_only.png", args.dpi)
    ...
    overlay_plus_ratio(ax_ytop, ax_yratio, ys, projy_real_s, projy_gen_s,
                       ...
                       pixel_axis=True, mid_line=MID_Y)
    save_figure(fig, out_dir / "centered_sum_profiles_ratio.png", args.dpi, tight=False)

    # ==================== Canvas 7: Random samples (real vs generated) — coppie con scala comune ====================
    n_total = reals.shape[0]
    ...
    for k, idx in enumerate(sel):
        ...
        ax_l = axs[row, col_pair]
        ax_r = axs[row, col_pair + 1]

        vmax_pair = float(max(r.max(initial=0.0), g.max(initial=0.0)))
        norm_pair = Normalize(vmin=0.0, vmax=max(vmax_pair, 1.0))

        im_l = _imshow_mappable(ax_l, r, f"Real #{k+1}", "x [Pixel]", "y [Pixel]",
                                pixel_axes=True, cmap=cmap, norm=norm_pair)
        im_r = _imshow_mappable(ax_r, g, f"Gen #{k+1}",  "x [Pixel]", "y [Pixel]",
                                pixel_axes=True, cmap=cmap, norm=norm_pair)
        _add_cbar_right_of(ax_l, im_l, label="Pixel counts")
    save_figure(fig, out_dir / "samples_real_vs_gen.png", args.dpi)
    ...
    # 2D Sliced WD: Occupancy maps (campionamento pesato)
    ...
    metrics_lines.append(f"SlicedWD_2D occupancy: {swd_occ:.6g}")

    # 2D Sliced WD: Sum maps (campionamento pesato)
    ...
    metrics_lines.append(f"SlicedWD_2D sum: {swd_sum:.6g}")

    # NUOVO: Sliced WD in alta dimensione su immagini intere (ℝ^(H·W))
    swd_imgs = _sliced_wd_images(reals, gens, n_proj=int(args.swd_img_proj), max_n=int(args.swd_img_maxn), seed=42)
    metrics_lines.append(f"SlicedWD_images_HD: {swd_imgs:.6g}")

    # Entropie (media,var) per-matrice
    ent_real_mean, ent_real_var = _entropy_per_image(reals)
    ent_gen_mean,  ent_gen_var  = _entropy_per_image(gens)
    metrics_lines.append(f"Entropy real: mean={ent_real_mean:.6g}, var={ent_real_var:.6g}")
    metrics_lines.append(f"Entropy gen:  mean={ent_gen_mean:.6g}, var={ent_gen_var:.6g}")
